...
def clip_data(data):
	data = data[data['x'] > min(data['x'] + 5.0)]
	data = data[data['x'] < max(data['x'] - 5.0)]

	data = data[data['y'] > min(data['y'] + 5.0)]
	data = data[data['y'] < max(data['y'] - 5.0)]
	return data
...
def Environment_classifier(Data, K):
	...
	points = np.array([Data['id'],Data['x'],Data['y'],Data['z']]).T


	all_data = np.array([Data['x'],Data['y'],Data['z']]).T
	            ...
	            for j in range(len(nearby_gal)):
	                ...
	                if (nearby_gal[j] in all_filament_gal):
	                        ...
	                        if (all_filament_gal_dist[ind] < dist):
	                            ...
	                            all_filament_gal_dist_tot_fil[ind] = lengths[q]
	                        ...
	                        all_filament_gal_dist_tot_fil.append(lengths[q])

	            
	            fil_to_plot.append(f)
	...
	with open(direc+'/fil_to_plot.txt', 'w') as f:
	    for item in fil_to_plot:
	        f.write("%s\n" % item)
